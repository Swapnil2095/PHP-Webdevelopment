  ...
  foreach ($_SESSION as $name => $value) {
    if($value > 0){
      if(substr($name, 0, 8) == "product_"){
          ...
          $send_order = query("INSERT INTO orders (order_amount, order_transaction,order_status,order_currency) VALUES ('{$amounnt}', '{$transaction}', '{$status}', '{$currency}')");
          $last_id = last_id();
          echo $last_id;
          confirm($send_order);
